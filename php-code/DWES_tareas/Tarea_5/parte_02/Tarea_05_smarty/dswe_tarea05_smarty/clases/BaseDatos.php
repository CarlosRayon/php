...
class BaseDatos
{
    ...
    /* Metodo que comprueba si es ciclo comun o no(consulta preparada) */

    //Le paso cod del ciclo pasado mediante
    public static function esCicloComun($cod)
    {
        $conexion = self::conexion(); //Establecemos conexion

        try {

            $query = "SELECT ciclo FROM `modulo` WHERE `cod` = :cod"; //Creo sentencia
            ...
            $fila = $PDOstmt->fetch(); //Capturo el dato devuelto 

            if ($fila['ciclo'] === "COM") {//Si el ciclo del resultado es igual a COM significa que es un modulo comun
                return true;
            } else {
                return false;
            }
    ...
    public static function alumnosMatriculados($codigo)
    {
        ...
        $arrayConsulta = Array();
        try {
            //Consigo el numero del contador que tenga el modulo eleguido en maestra.php
            $query = "SELECT contador FROM `listamatricula` WHERE `modulo` = :cod";
            ...
            $contador = $PDOstmt->fetch(); //Guardo el contado del modulo
            //Con el numero del contador saco los alumnos que tengan este contador
            $query = "SELECT * FROM `matricula` WHERE `contador` = :numero";
            //Hago la consulta preparada
            $PDOstmt = $conexion->prepare($query);
            $PDOstmt->bindParam(':numero', $conta);
